<script>
  ...
  async function loadJovensAvaliados() {
    loading = true;
    error = null;
    
    try {
      // Primeiro, buscar IDs dos jovens que têm avaliações
      ...
      // Extrair IDs únicos dos jovens avaliados
      const jovensAvaliadosIds = [...new Set(avaliacoesData.map(a => a.jovem_id))];
      
      if (jovensAvaliadosIds.length === 0) {
        jovens = [];
        return;
      }
      
      // Buscar dados dos jovens avaliados
      let query = supabase
        .from('jovens')
        .select(`
          *,
        ...
        `)
        .in('id', jovensAvaliadosIds);
      
      // 🔧 APLICAR FILTROS BASEADOS NO NÍVEL DE ACESSO
      const userLevel = $userProfile?.nivel;
      const userId = $userProfile?.id;
      
      console.log('🔍 DEBUG - Carregando jovens avaliados:', { userLevel, userId, estado_id: $userProfile?.estado_id });
      ...
      jovens = data.map(jovem => ({
        ...
        tem_avaliacoes: true // Jovens avaliados têm avaliações
      }));
      
      console.log('🔍 DEBUG - Jovens carregados:', jovens.length);
      
    } catch (err) {
      error = err.message;
      console.error('Erro ao carregar jovens avaliados:', err);
    } finally {
      loading = false;
    }
  }
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between py-4 sm:py-0 sm:h-16 space-y-4 sm:space-y-0">
        ...
        <div class="flex items-center space-x-3 sm:space-x-4">
          ...
          <div class="min-w-0 flex-1">
            <h1 class="text-xl sm:text-2xl font-bold text-gray-900 truncate">Jovens Avaliados</h1>
            <p class="text-xs sm:text-sm text-gray-500">{jovens.length} jovens avaliados encontrados</p>
          </div>
        </div>
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
    ...
    {:else if error}
      <div class="text-center py-8 sm:py-12 px-4">
        ...
        <Button on:click={loadJovensAvaliados} variant="outline" class="w-full sm:w-auto">
          Tentar Novamente
        </Button>
      </div>
    {:else if jovens.length === 0}
      <div class="text-center py-8 sm:py-12 px-4">
        ...
        <h3 class="text-base sm:text-lg font-semibold text-gray-900 mb-2">Nenhum jovem avaliado</h3>
        <p class="text-sm sm:text-base text-gray-500 mb-4 max-w-md mx-auto">Ainda não há jovens que foram avaliados</p>
        <Button href="/jovens/cadastrar" variant="primary" class="w-full sm:w-auto">
