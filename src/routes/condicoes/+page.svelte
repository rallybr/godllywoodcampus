<script>
  ...
  async function loadJovensPorCondicao() {
    loading = true;
    error = null;
    ...
    try {
      ...
      const { data, error: fetchError } = await query.order('nome_completo');
      ...
      error = err.message;
      console.error('Erro ao carregar jovens por condição:', err);
    } finally {
      loading = false;
    }
  }
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
    ...
    {:else if error}
      <div class="text-center py-8 sm:py-12 px-4">
        ...
        <Button on:click={loadJovensPorCondicao} variant="outline" class="w-full sm:w-auto">
          Tentar Novamente
        </Button>
      </div>
    {:else if jovens.length === 0}
      <div class="text-center py-8 sm:py-12 px-4">
        ...
        <p class="text-sm sm:text-base text-gray-500 mb-4 max-w-md mx-auto">Não há jovens cadastrados na condição "{condicaoNome}"</p>
        <Button href="/jovens/cadastrar" variant="primary" class="w-full sm:w-auto">
