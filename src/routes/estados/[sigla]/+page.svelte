<script>
  ...
  onMount(async () => {
    if (!$user) {
      ...
      siglaEstado = window.location.pathname.split('/').pop();
  ...
  async function loadJovensPorEstado() {
    ...
    try {
      ...
      // Agora buscar jovens pelo estado_id
      let query = supabase
        .from('jovens')
        .select(`
          *,
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between py-4 sm:py-0 sm:h-16 space-y-4 sm:space-y-0">
        ...
        <div class="flex items-center space-x-3 sm:space-x-4">
          ...
          <div class="min-w-0 flex-1">
            <h1 class="text-xl sm:text-2xl font-bold text-gray-900 truncate">{nomeEstado}</h1>
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
    ...
    {:else if error}
      <div class="text-center py-8 sm:py-12 px-4">
        ...
        <Button on:click={loadJovensPorEstado} variant="outline" class="w-full sm:w-auto">
          Tentar Novamente
        </Button>
      </div>
    {:else if jovens.length === 0}
      <div class="text-center py-8 sm:py-12 px-4">
        ...
        <p class="text-sm sm:text-base text-gray-500 mb-4 max-w-md mx-auto">Não há jovens cadastrados no estado "{nomeEstado}"</p>
