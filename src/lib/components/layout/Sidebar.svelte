<script>
  ...
  const menuItems = [
    ...
    { name: 'Ponto de Vista', href: '/ponto-de-vista', icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z', color: 'text-violet-600', nacionalOnly: true },
  ...
  $: profile = $userProfile;
  $: isJovem = profile?.nivel === 'jovem' || getUserLevelName($userProfile) === 'Jovem';
  ...
  $: filteredMenuItems = isJovem
    ...
    : menuItems.filter((item) => {
        if (item.adminOnly) return profile?.nivel === 'administrador';
        if (item.nacionalOnly) {
          return ['administrador', 'lider_nacional_iurd', 'lider_nacional_fju'].includes(profile?.nivel);
        }
        // Garantia extra: nunca exibir itens sensíveis para jovem
        if (profile?.nivel === 'jovem') return false;
        return true;
      });
