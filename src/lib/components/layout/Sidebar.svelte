<script>
  ...
  const menuItems = [
    ...
    },
    {
      ...
      submenu: [
        ...
        {
          name: 'Jovens',
          href: '/relatorios/jovens',
          icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5-9a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0z'
        },
        {
          name: 'Personalizados',
          href: '/relatorios/personalizados',
  ...
  $: filteredMenuItems = isJovem
    ...
    : menuItems.filter(item => {
        if (item.adminOnly) {
          return $userProfile?.user_roles?.[0]?.roles?.nome === 'Administrador';
        }
        return true;
      });
...
<!-- Sidebar -->
<aside class="fb-card rounded-none border-0 border-r transition-all duration-300 {showSidebar ? 'w-64' : 'w-0'} overflow-hidden" style="background-color: var(--fb-white);">
