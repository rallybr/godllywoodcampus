<script>
  ...
  const menuItems = [
    ...
    { name: 'Progresso', href: '/progresso', icon: 'M13 10V3L4 14h7v7l9-11h-7z', color: 'text-cyan-600' },
  ...
  $: isJovem = hasRole('jovem')($userProfile);
  
  $: filteredMenuItems = isJovem
    ? [ { name: 'Meu Perfil', href: '/profile', icon: 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z', color: 'text-gray-600' }, { name: 'Cadastro', href: '/jovens/cadastrar', icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1', color: 'text-blue-600' } ]
    : menuItems.filter(item => item.adminOnly ? $userProfile?.user_roles?.[0]?.roles?.nome === 'Administrador' : true);
...
<!-- Sidebar -->
<aside class="fixed inset-y-0 left-0 z-50 w-64 bg-white transform transition-transform duration-300 ease-in-out {showSidebar ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 md:w-64 border-r border-gray-200 shadow-lg md:shadow-none">
