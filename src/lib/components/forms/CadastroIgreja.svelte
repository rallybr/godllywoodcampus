<script>
  ...
  import { loadInitialData, estados, blocos, regioes, loadBlocos, loadRegioes, clearHierarchy } from '$lib/stores/geographic';
  ...
  onMount(async () => {
    ...
    // Converter regiões para formato do select
    regioes.subscribe(regioesData => {
      regioesOptions = regioesData.map(regiao => ({
        value: regiao.id,
        label: regiao.nome
      }));
    });
  ...
  async function handleEstadoChange(event) {
    ...
    formData.regiao_id = ''; // Limpar região selecionada
  ...
  function handleRegiaoChange(event) {
    formData.regiao_id = event.detail.value;
  }
  ...
  function validateForm() {
    ...
    if (!formData.regiao_id) {
      error = 'Por favor, selecione uma região';
      return false;
    }
    if (!formData.nome.trim()) {
      error = 'Por favor, informe o nome da igreja';
      return false;
    }
    if (!formData.endereco.trim()) {
      error = 'Por favor, informe o endereço da igreja';
      return false;
    }
    return true;
  }

  // Função para salvar igreja
  async function handleSubmit() {
    ...
    try {
      ...
      error = err.message || 'Erro ao salvar igreja';
    } finally {
      isLoading = false;
    }
  }
...
<div class="max-w-2xl mx-auto p-6">
  <Card>
    <div class="p-6">
      <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900 mb-2">Cadastrar Igreja</h1>
        <p class="text-gray-600">Preencha os dados para cadastrar uma nova igreja</p>
      </div>
