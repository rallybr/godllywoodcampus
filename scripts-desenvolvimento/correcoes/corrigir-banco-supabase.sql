...
-- 7. ATUALIZAR DADOS GEOGRÁFICOS COM NOMES REAIS
-- São Paulo
...
-- Política para permitir leitura de roles (necessário para o frontend)
CREATE POLICY "allow_read_all_roles" ON public.roles FOR SELECT USING (true);

-- Política para permitir leitura de dados geográficos
...
-- Política para permitir leitura de edições
CREATE POLICY "allow_read_all_edicoes" ON public.edicoes FOR SELECT USING (true);

-- Política para permitir leitura de jovens (temporária - para teste)
CREATE POLICY "allow_read_all_jovens" ON public.jovens FOR SELECT USING (true);
