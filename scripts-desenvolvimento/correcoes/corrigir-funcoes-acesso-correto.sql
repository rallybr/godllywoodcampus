-- CORREÇÃO DAS FUNÇÕES DE ACESSO BASEADA NO CAMPO NIVEL
-- Usando a lógica correta baseada no campo 'nivel' da tabela usuarios

-- 1. CORRIGIR FUNÇÃO can_access_jovem
...
SELECT 
  'Função can_access_jovem corrigida' as item,
  CASE WHEN EXISTS (
...
SELECT 
  'Função can_access_viagem_by_level corrigida' as item,
  CASE WHEN EXISTS (
    SELECT 1 FROM information_schema.routines 
