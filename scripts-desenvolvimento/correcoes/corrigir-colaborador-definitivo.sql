-- CORREÇÃO DEFINITIVA PARA COLABORADOR
-- Colaborador deve ver APENAS os jovens que ele cadastrou (não importa a localização)
...
BEGIN
  ...
  -- 7. COLABORADOR - Acesso APENAS aos jovens que cadastrou (SEM verificar localização)
...
-- 2. CORRIGIR FUNÇÃO can_access_viagem_by_level
...
SELECT 
  'Função can_access_jovem corrigida para colaborador' as item,
  CASE WHEN EXISTS (
...
SELECT 
  'Função can_access_viagem_by_level corrigida para colaborador' as item,
  CASE WHEN EXISTS (
    SELECT 1 FROM information_schema.routines 
