-- =====================================================
-- TESTE COM DEBUG PARA IDENTIFICAR PROBLEMA
-- =====================================================
-- Este script testa as functions com debug para identificar o problema
...
-- 5. TESTAR FUNCTION can_access_jovem COM DEBUG
...
-- 8. VERIFICAR SE O USUÁRIO TEM ACESSO ÀS FUNCTIONS
SELECT 
  'VERIFICAÇÃO DE PERMISSÕES' as diagnostico,
  has_function_privilege(current_user, 'public.can_access_jovem(uuid,uuid,uuid,uuid)', 'EXECUTE') as can_execute_jovem,
  has_function_privilege(current_user, 'public.can_access_viagem_by_level(uuid,uuid,uuid,uuid)', 'EXECUTE') as can_execute_viagem;
...
-- 10. VERIFICAR SE O USUÁRIO EXISTE NA TABELA usuarios
SELECT 
...
-- 11. VERIFICAR SE O USUÁRIO TEM PAPÉIS ATIVOS
SELECT 
...
-- 12. VERIFICAR SE O USUÁRIO TEM PAPÉIS COM NÍVEL HIERÁRQUICO
SELECT 
...
-- 13. VERIFICAR SE O USUÁRIO TEM PAPÉIS COM NÍVEL HIERÁRQUICO VÁLIDO
SELECT 
...
-- 14. VERIFICAR SE O USUÁRIO TEM PAPÉIS COM NÍVEL HIERÁRQUICO VÁLIDO
SELECT 
  'VERIFICAÇÃO DE NÍVEIS HIERÁRQUICOS VÁLIDOS' as diagnostico,
  COUNT(*) as total_papeis,
  COUNT(CASE WHEN r.nivel_hierarquico BETWEEN 1 AND 8 THEN 1 END) as papeis_com_nivel_valido,
...
-- 15. VERIFICAR SE O USUÁRIO TEM PAPÉIS COM NÍVEL HIERÁRQUICO VÁLIDO
SELECT 
  'VERIFICAÇÃO DE NÍVEIS HIERÁRQUICOS VÁLIDOS' as diagnostico,
  COUNT(*) as total_papeis,
  COUNT(CASE WHEN r.nivel_hierarquico BETWEEN 1 AND 8 THEN 1 END) as papeis_com_nivel_valido,
