-- =====================================================
-- CORREÇÃO SIMPLES: Apenas atualizar aprovar_jovem_multiplo
-- =====================================================
-- Problema: Erro "column reference jovem_id is ambiguous"
-- Solução: Atualizar apenas a função aprovar_jovem_multiplo sem chamar atualizar_status_jovem

-- 1. Criar função can_access_jovem_com_associacoes (versão simplificada)
...
-- 2. Atualizar função aprovar_jovem_multiplo (SEM chamar atualizar_status_jovem)
...
BEGIN
  ...
  -- ✅ CORREÇÃO: Usar a nova função que considera associações
...
COMMENT ON FUNCTION public.can_access_jovem_com_associacoes IS 'v2.1.0 - Verificação de acesso a jovens incluindo associações (versão simplificada)';
COMMENT ON FUNCTION public.aprovar_jovem_multiplo IS 'v2.1.0 - Sistema de aprovações múltiplas com suporte a associações (sem atualizar_status_jovem)';
